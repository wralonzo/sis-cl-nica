...
#buscar{
 ...
 <!--end of modal-->
    ...
    <th>Fecha</th>
      ...
      <th>Usuario Modificador</th> <!-- Agregamos esta columna -->
...
$query = mysqli_query($con, "SELECT m.nombre AS medico, p.nombre AS paciente, c.fecha AS fecha_cita,
                     h.fecha_actualizacion, c.observaciones, h.id_usuario, h.nombre_usuario, u.usuario
                     FROM cita_historial h 
                     INNER JOIN cita c ON c.id_cita = h.id_cita 
                     INNER JOIN usuario m ON c.id_medico = m.id 
                     INNER JOIN usuario p ON p.id = c.id_paciente 
                     INNER JOIN usuario u ON u.id = h.id_usuario") or die(mysqli_error());
            $i = 0;
