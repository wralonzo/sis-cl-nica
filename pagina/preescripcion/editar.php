...
?>
 ...
 <!--end of modal-->
              ...
              <a class="btn btn-warning btn-print" href="preescripcion.php"  
                style="height:25%; width:15%; font-size: 12px " role="button">Regresar</a>
...
<?php
   // $branch=$_SESSION['branch'];
    $query=mysqli_query($con,"select m.nombre as  medico,p.nombre as  paciente,c.fecha,
    c.medicina,c.estado_cita,c.id_cita from cita c inner join usuario m on c.id_medico = m.id inner join usuario
     p on p.id = c.id_paciente
...
?>
              ...
              $queryc=mysqli_query($con,"select * from usuario  where tipo='medico'")or die(mysqli_error());
                ...
                ?>
                                     <option value="<?php echo $rowc['id'];?>" <?php if($id_medico==$rowc['id'])
                                      {echo "selected"; }?>><?php echo $rowc['nombre'].'  '.$rowc['apellido'];?></option> 
     ...
     <div class="row">
                    <div class="col-md-3 btn-print">
                      <div class="form-group">
                        <label for="date" >Agregar Receta</label>
                   ...
                   <textarea  class="form-control" id="medicina" name="medicina"><?php echo $row['medicina'];?></textarea>
